...
function loadState() {
    ...
    if (data) {
        ...
        a.forEach(x => todos.push(new TodoItem(x.text, x.completed, x._priority)));
    }
...
function createTodoElement(todo) {
    ...
    spanPrio.innerHTML = `P: ${todo.priority}`;

    const btnPrioUp = document.createElement("button");
    btnPrioUp.innerText = "+";
    btnPrioUp.classList.add("todo-item-prio-btn");
    if (todo.priority >= 5 || todo.completed) {
        btnPrioUp.disabled = true;
    }
    btnPrioUp.addEventListener('click', _ => {
        todo.increasePriority();
        render();
    });

    const btnPrioDown = document.createElement("button");
    btnPrioDown.innerText = "-";
    btnPrioDown.classList.add("todo-item-prio-btn");
    if (todo.priority <= 1 || todo.completed) {
        btnPrioDown.disabled = true;
    }
        
    btnPrioDown.addEventListener('click', _ => {
    ...
    listItem.append(cbDone, spanPrio, btnPrioUp, btnPrioDown, todoText, deleteBtn);
    return listItem;
}
